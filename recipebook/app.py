...
# Resolve paths relative to this file so they work on Vercel
...
# Use /tmp on Vercel (read-only filesystem everywhere else)
...
def parse_recipe(caption: str) -> dict:
    ...
    for line in lines[1:]:
        ...
        bullet = re.match(r'^[-\u2022\u2714\u2705\U0001F538\U0001F539\u25B6\uFE0F\u27A1\uFE0F\u27A4*]\s*', clean)
        if bullet:
...
# Initialise DB on first import (works for both local dev and Vercel cold start)
init_db()
